...
func TestCreateValidatesOfferingAndParent(t *testing.T) {
    ...
    for _, status := range []event.Status{event.StatusDraft, event.StatusPublished, event.StatusActive, event.StatusSuspended} {
        t.Run(string(status), func(t *testing.T) {
            mutation, err := Create(event.Event{Status: status}, input)
            if err != nil {
    ...
    for _, status := range []event.Status{event.StatusClosed, event.StatusArchived, "UNKNOWN"} {
        t.Run("rejected_"+string(status), func(t *testing.T) {
            if _, err := Create(event.Event{Status: status}, input); !errors.Is(err, ErrInvalidParentState) {
    ...
    for _, test := range tests {
        t.Run(test.name, func(t *testing.T) {
            if _, err := Create(event.Event{Status: event.StatusDraft}, test.input); !errors.Is(err, ErrInvalidInput) {
...
func TestUpdateHonorsCommercialMutationPolicy(t *testing.T) {
    ...
    tests := []struct {
        ...
        {name: "draft", parent: event.StatusActive, status: StatusDraft},
        {name: "unavailable", parent: event.StatusSuspended, status: StatusUnavailable},
        {name: "published frozen", parent: event.StatusActive, status: StatusPublished, wantErr: ErrConfigurationFrozen},
        {name: "archived immutable", parent: event.StatusActive, status: StatusArchived, wantErr: ErrImmutable},
        {name: "closed event", parent: event.StatusClosed, status: StatusDraft, wantErr: ErrInvalidParentState},
    }
    for _, test := range tests {
        t.Run(test.name, func(t *testing.T) {
            ...
            mutation, err := Update(event.Event{Status: test.parent}, current, input)
            if test.wantErr != nil {
    ...
    if _, err := Update(event.Event{Status: event.StatusActive}, testOffering(StatusDraft), UpdateInput{ExpectedVersion: 2, Name: stringPointer("Changed")}); !errors.Is(err, ErrStaleVersion) {
...
func TestLifecyclePreservesFirstPublicationAndCleanup(t *testing.T) {
    ...
    republished, err := Publish(event.Event{Status: event.StatusSuspended}, current, PublishInput{ExpectedVersion: 1, OccurredAt: publication.Add(24 * time.Hour)})
    if err != nil {
    ...
    if _, err := Publish(event.Event{Status: event.StatusDraft}, testOffering(StatusDraft), PublishInput{ExpectedVersion: 1, OccurredAt: publication}); !errors.Is(err, ErrInvalidParentState) {
        t.Fatalf("Publish() error = %v, want invalid parent", err)
    }
    if _, err := Publish(event.Event{Status: event.StatusPublished}, testOffering(StatusPublished), PublishInput{ExpectedVersion: 1, OccurredAt: publication}); !errors.Is(err, ErrInvalidTransition) {
...
func TestPublicVisibilityAndAvailability(t *testing.T) {
    if !IsPublic(event.Event{Status: event.StatusActive}, testOffering(StatusPublished)) {
        t.Fatal("active published offering should be public")
    }
    if IsPublic(event.Event{Status: event.StatusSuspended}, testOffering(StatusPublished)) || IsPublic(event.Event{Status: event.StatusActive}, testOffering(StatusUnavailable)) {
