...
func Create(parent event.Event, input CreateInput) (Mutation, error) {
...
func Update(parent event.Event, current Offering, input UpdateInput) (Mutation, error) {
...
func IsPublic(parent event.Event, offering Offering) bool {
    return parent.Status == event.StatusActive && offering.Status == StatusPublished
}
...
func allowsPublish(status event.Status) bool {
    switch status {
    case event.StatusPublished, event.StatusActive, event.StatusSuspended:
        return true
    default:
        return false
    }
