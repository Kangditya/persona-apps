...
func NewRelationships(input RelationshipsInput) (Relationships, error) {
    ...
    declared := map[string]identity.Party{purchaser.Ref: purchaser.Party}
    ...
    for _, inputParticipant := range input.Participants {
        ...
        nameOnlyParty, nameErr := identity.NewParty(identity.CreateInput{Type: identity.PartyTypePerson, DisplayName: inputParticipant.DisplayName})
        if nameErr != nil {
...
func newDeclaredParty(input PartyDeclarationInput) (DeclaredParty, error) {
    ...
    party, err := identity.NewParty(input.Party)
    if err != nil {
