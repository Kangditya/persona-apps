...
func TestRepositoryPostgreSQL(t *testing.T) {
    ...
    create := func(year int, suffix string) eventdomain.Event {
        t.Helper()
        mutation, err := eventdomain.Create(eventdomain.CreateInput{EventYear: year, Name: prefix + "-" + suffix})
        if err != nil {
            t.Fatal(err)
        }
    ...
    if _, err := repository.Create(ctx, eventdomain.Event{EventYear: first.EventYear, Name: prefix + "-duplicate", Status: eventdomain.StatusDraft}); !errors.Is(err, eventdomain.ErrDuplicateYear) {
        t.Fatalf("duplicate event error = %v, want duplicate year", err)
    }

    page, err := repository.List(ctx, eventdomain.ListInput{Limit: 1})
    if err != nil {
        t.Fatal(err)
    }
    ...
    nextPage, err := repository.List(ctx, eventdomain.ListInput{Limit: 1, Cursor: page.NextCursor})
    ...
    next, err := repository.List(ctx, eventdomain.ListInput{Limit: MaxListLimit, Cursor: thirdCursor})
    if err != nil {
        t.Fatal(err)
    }
    ...
    if _, err := repository.List(ctx, eventdomain.ListInput{Limit: 2, Cursor: "not-a-cursor"}); !errors.Is(err, eventdomain.ErrInvalidCursor) {
        t.Fatalf("invalid cursor error = %v", err)
    }

    updatedMutation, err := eventdomain.Update(first, eventdomain.UpdateInput{ExpectedVersion: first.Version, Name: stringPointer(prefix + "-updated")})
    if err != nil {
        t.Fatal(err)
    }
    ...
    if _, err := repository.Save(ctx, updatedMutation.Event, first.Version); !errors.Is(err, eventdomain.ErrStaleVersion) {
    ...
    rollbackErr := database.Within(ctx, db, func(transaction *sql.Tx) error {
        mutation, err := eventdomain.Create(eventdomain.CreateInput{EventYear: baseYear + 4, Name: prefix + "-rollback"})
        if err != nil {
            return err
        }
    ...
    if _, err := repository.Get(ctx, rolledBackID); !errors.Is(err, eventdomain.ErrNotFound) {
        t.Fatalf("rolled back event lookup error = %v", err)
    }

    if _, err := db.ExecContext(ctx, `INSERT INTO qurban_events (event_year, name, status, participant_quota) VALUES ($1, $2, 'DRAFT', $3)`, baseYear+5, prefix+"-unsafe-quota", eventdomain.MaxSafeInteger+1); err == nil {
        t.Fatal("database accepted an unsafe eventdomain.Event quota")
    }
}

func assertConcurrentActivation(ctx context.Context, db *sql.DB, repository *Repository, first, second eventdomain.Event) error {
    publish := func(value eventdomain.Event) (eventdomain.Event, error) {
        mutation, err := eventdomain.Publish(value, eventdomain.TransitionInput{ExpectedVersion: value.Version})
        if err != nil {
    ...
    for _, value := range []eventdomain.Event{first, second} {
        group.Add(1)
        go func(value eventdomain.Event) {
            defer group.Done()
            <-start
            errorsByEvent <- database.Within(ctx, db, func(transaction *sql.Tx) error {
                ...
                mutation, err := eventdomain.Activate(locked, eventdomain.TransitionInput{ExpectedVersion: locked.Version})
                if err != nil {
                    return err
                }
    ...
    for err := range errorsByEvent {
        ...
        case err == nil:
            succeeded++
        case errors.Is(err, eventdomain.ErrActiveConflict):
            conflicted++
        default:
...
func containsEvent(events []eventdomain.Event, id string) bool {
