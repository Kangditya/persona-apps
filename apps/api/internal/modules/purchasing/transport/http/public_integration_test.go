...
func TestPublicPurchaseCheckoutRetriesDuplicateReferencePostgreSQL(t *testing.T) {
    ...
    service := purchasingapplication.NewService(database, func(database platformdb.DBTX) purchasingapplication.Repository {
        return purchasingpersistence.NewRepository(database)
    }, identity.Service(), purchasingpersistence.LockCheckoutForOffering, purchasingpersistence.Reserve)
    handler := NewHandler(database, service, cipher, slog.New(slog.NewTextHandler(io.Discard, nil)))
    ...
    service.SetRandomSource(random)
    router := gin.New()
