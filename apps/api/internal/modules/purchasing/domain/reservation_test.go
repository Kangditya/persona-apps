...
func TestValidateCheckoutSource(t *testing.T) {
    ...
    source := CheckoutSource{
        Event:      eventdomain.Event{ID: "event-1", Status: eventdomain.StatusActive},
        Offering:   offeringdomain.Offering{ID: "offering-1", EventID: "event-1", Status: offeringdomain.StatusPublished, ParticipantCapacity: 1},
        OccurredAt: now,
    }
    ...
    cases := []struct {
        ...
        {"inactive event", func(value *CheckoutSource) { value.Event.Status = eventdomain.StatusSuspended }, ErrStateConflict},
        {"unpublished offering", func(value *CheckoutSource) { value.Offering.Status = offeringdomain.StatusDraft }, ErrStateConflict},
        {"before opening", func(value *CheckoutSource) {
            opening := now.Add(time.Second)
            value.Event.RegistrationOpensAt = &opening
        }, ErrStateConflict},
...
func TestValidateReservationPurchase(t *testing.T) {
    source := CheckoutSource{
        Event:    eventdomain.Event{ID: "event-1"},
        Offering: offeringdomain.Offering{ID: "offering-1", EventID: "event-1", ParticipantCapacity: 2},
    }
