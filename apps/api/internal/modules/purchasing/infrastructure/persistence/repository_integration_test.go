...
func TestRepositoryPostgreSQL(t *testing.T) {
    ...
    partyRepository := identitypersistence.NewRepository(database)
    purchaserEmail := prefix + "-purchaser@example.test"
    purchaser, err := identitydomain.NewParty(identitydomain.CreateInput{Type: identitydomain.PartyTypePerson, DisplayName: "Purchaser", Email: &purchaserEmail})
    if err != nil {
        t.Fatal(err)
    }
    ...
    payer, err := identitydomain.NewParty(identitydomain.CreateInput{Type: identitydomain.PartyTypePerson, DisplayName: "Payer"})
    if err != nil {
        t.Fatal(err)
    }
    ...
    rolledBack, err := NewPurchase(CreateInput{
        EventID: purchase.EventID, PurchaseRef: prefix + "-rollback", PurchaserPartyID: purchase.PurchaserPartyID,
        PayerPartyID: *purchase.PayerPartyID, OfferingID: purchase.OfferingID,
        OfferingNameSnapshot: purchase.OfferingNameSnapshot, OfferingKindSnapshot: purchase.OfferingKindSnapshot,
        OfferingUnitPriceMinor: purchase.OfferingUnitPriceMinor, ParticipantCapacitySnapshot: int64(purchase.ParticipantCapacitySnapshot),
        TotalAmountMinor: purchase.TotalAmountMinor, CurrencyCode: purchase.CurrencyCode,
        Participants:    []PurchaseParticipantInput{{PartyID: purchase.Participants[0].PartyID, DisplayName: purchase.Participants[0].DisplayName}, {PartyID: purchase.Participants[1].PartyID, DisplayName: purchase.Participants[1].DisplayName}},
        AccessTokenHash: rollbackHash[:],
    })
    if err != nil {
        t.Fatal(err)
    }
