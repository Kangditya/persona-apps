...
type ListResult = purchasingdomain.ListResult
type Channel = purchasingdomain.Channel
type Status = purchasingdomain.Status

var (
    ErrNotFound           = purchasingdomain.ErrNotFound
    ErrInvalidInput       = purchasingdomain.ErrInvalidInput
    ErrInvalidCursor      = purchasingdomain.ErrInvalidCursor
    ErrConflict           = purchasingdomain.ErrConflict
    ErrDuplicateReference = purchasingdomain.ErrDuplicateReference
)

const (
    StatusDraft          = purchasingdomain.StatusDraft
    StatusPendingPayment = purchasingdomain.StatusPendingPayment
    StatusPaid           = purchasingdomain.StatusPaid
    StatusEligible       = purchasingdomain.StatusEligible
    StatusCancelled      = purchasingdomain.StatusCancelled
)
...
type purchaseDetailRow struct {
    purchaseRow
    PurchaserID          string
    PurchaserDisplayName string
    PayerID              sql.NullString
    PayerDisplayName     sql.NullString
}
...
func scanPurchase(value scanner) (Purchase, error) {
    ...
    return row.purchase(), nil
}

func scanDetail(value scanner) (Detail, error) {
    var row purchaseDetailRow
    if err := value.Scan(row.destinations()...); err != nil {
        return Detail{}, err
    }
...
func scanParticipant(value scanner) (Participant, error) {
    ...
    if err := value.Scan(&row.ID, &row.PartyID, &row.SequenceNo, &row.DisplayName); err != nil {
        return Participant{}, err
    }
    return row.participant(), nil
}

func (row *purchaseRow) destinations() []any {
    return []any{
        &row.ID,
        &row.EventID,
        &row.PurchaseRef,
        &row.Channel,
        &row.PurchaserPartyID,
        &row.PayerPartyID,
        &row.OfferingID,
        &row.OfferingNameSnapshot,
        &row.OfferingKindSnapshot,
        &row.OfferingUnitPriceMinor,
        &row.ParticipantCapacitySnapshot,
...
func (row *purchaseDetailRow) destinations() []any {
    destinations := row.purchaseRow.destinations()
    return append(destinations, &row.PurchaserID, &row.PurchaserDisplayName, &row.PayerID, &row.PayerDisplayName)
}

func (row purchaseRow) purchase() Purchase {
    return Purchase{
        ID:                          row.ID,
        EventID:                     row.EventID,
        PurchaseRef:                 row.PurchaseRef,
        Channel:                     Channel(row.Channel),
        PurchaserPartyID:            row.PurchaserPartyID,
        PayerPartyID:                nullableString(row.PayerPartyID),
        OfferingID:                  row.OfferingID,
        OfferingNameSnapshot:        row.OfferingNameSnapshot,
        OfferingKindSnapshot:        row.OfferingKindSnapshot,
        OfferingUnitPriceMinor:      row.OfferingUnitPriceMinor,
        ParticipantCapacitySnapshot: row.ParticipantCapacitySnapshot,
...
func (row purchaseDetailRow) detail() Detail {
    detail := Detail{
        Purchase:  row.purchaseRow.purchase(),
        Purchaser: PartySummary{ID: row.PurchaserID, DisplayName: row.PurchaserDisplayName},
    }
    if row.PayerID.Valid {
        detail.Payer = &PartySummary{ID: row.PayerID.String, DisplayName: row.PayerDisplayName.String}
    }
    return detail
}

func (row participantRow) participant() Participant {
    return Participant{ID: row.ID, PartyID: nullableString(row.PartyID), SequenceNo: row.SequenceNo, DisplayName: row.DisplayName}
}
