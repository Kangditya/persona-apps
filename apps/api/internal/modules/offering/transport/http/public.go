...
func (handler publicHandler) list(c *gin.Context) {
    ...
    if handler.reader == nil {
        writePublicOfferingError(c, handler.logger, httpx.ErrDependencyUnavailable)
        return
    }
    result, err := handler.reader.ListPublic(c.Request.Context(), eventID, input)
    if err != nil {
        writePublicOfferingError(c, handler.logger, err)
        return
    }
    data := make([]publicOffering, 0, len(result.Offerings))
    for _, value := range result.Offerings {
        if value.Offering.Status != offeringdomain.StatusPublished {
            writePublicOfferingError(c, handler.logger, fmt.Errorf("unexpected public offering status %q", value.Offering.Status))
            return
        }
        data = append(data, toPublicOffering(value))
    }
    c.JSON(http.StatusOK, publicOfferingListResponse{Data: data, Page: publicOfferingPage{Limit: result.Limit, NextCursor: result.NextCursor}})
}

func (handler publicHandler) get(c *gin.Context) {
    ...
    result, err := handler.reader.GetPublic(c.Request.Context(), offeringID)
    if err != nil {
        writePublicOfferingError(c, handler.logger, err)
        return
    }
    if result.Offering.Status != offeringdomain.StatusPublished {
        writePublicOfferingError(c, handler.logger, fmt.Errorf("unexpected public offering status %q", result.Offering.Status))
        return
    }
...
func publicUUID(c *gin.Context, parameter string) (string, bool) {
    raw, err := httpx.CanonicalUUID(c.Param(parameter))
    if err != nil {
...
func publicListInput(c *gin.Context) (offeringdomain.ListInput, error) {
    ...
    if limits, found := values["limit"]; found {
        ...
        if err != nil || limit < 1 || limit > offeringdomain.MaxListLimit {
            return offeringdomain.ListInput{}, offeringdomain.ErrInvalidInput
        }
        input.Limit = limit
    }
...
func toPublicOffering(value offeringdomain.CatalogueOffering) publicOffering {
    ...
    return publicOffering{
        ...
        ParticipantCapacity: offering.ParticipantCapacity, AvailableParticipantUnits: copyInt64(value.AvailableParticipantUnits), Status: offering.Status,
    }
}
...
func writePublicOfferingError(c *gin.Context, logger *slog.Logger, err error) {
    ...
    switch {
    case errors.Is(err, offeringdomain.ErrNotFound):
        httpx.WriteError(c, http.StatusNotFound, "not_found", "resource not found", requestID, nil)
    case errors.Is(err, offeringdomain.ErrInvalidInput), errors.Is(err, offeringdomain.ErrInvalidCursor):
