...
func TestRepositoryPostgreSQL(t *testing.T) {
    ...
    eventRepository := eventpersistence.NewRepository(db)
    ...
    suspended, err := eventdomain.Suspend(parent, eventdomain.TransitionInput{ExpectedVersion: parent.Version})
    if err != nil {
        t.Fatal(err)
    }
...
func createActiveEvent(t *testing.T, ctx context.Context, repository *eventpersistence.Repository, year int, name string) eventdomain.Event {
    t.Helper()
    quota := int64(10)
    createdMutation, err := eventdomain.Create(eventdomain.CreateInput{EventYear: year, Name: name, ParticipantQuota: &quota})
    if err != nil {
        t.Fatal(err)
    }
    ...
    publishedMutation, err := eventdomain.Publish(created, eventdomain.TransitionInput{ExpectedVersion: created.Version})
    if err != nil {
        t.Fatal(err)
    }
    ...
    activeMutation, err := eventdomain.Activate(published, eventdomain.TransitionInput{ExpectedVersion: published.Version})
    if err != nil {
        t.Fatal(err)
    }
...
func insertReservation(t *testing.T, ctx context.Context, db *sql.DB, parent eventdomain.Event, value Offering, partyID, prefix string, number int, status string, units int) {
    t.Helper()
...
func assertConcurrentUpdate(ctx context.Context, db *sql.DB, repository *Repository, parent eventdomain.Event, current Offering, prefix string) error {
