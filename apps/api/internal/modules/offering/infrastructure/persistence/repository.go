...
func (repository *Repository) Create(ctx context.Context, value offeringdomain.Offering) (offeringdomain.Offering, error) {
    ...
    if err != nil {
        return offeringdomain.Offering{}, fmt.Errorf("insert offering: %w", translateError(err))
    }
    return created, nil
}

func (repository *Repository) Get(ctx context.Context, id string) (offeringdomain.Offering, error) {
    return repository.get(ctx, `SELECT `+offeringColumns+` FROM offerings WHERE id = $1`, id)
}

func (repository *Repository) GetForUpdate(ctx context.Context, id string) (offeringdomain.Offering, error) {
    return repository.get(ctx, `SELECT `+offeringColumns+` FROM offerings WHERE id = $1 FOR UPDATE`, id)
}

func (repository *Repository) List(ctx context.Context, eventID string, input offeringdomain.ListInput) (offeringdomain.ListResult, error) {
    limit, cursor, err := offeringdomain.NormalizeListInput(input)
    if err != nil {
        return offeringdomain.ListResult{}, err
    }
    ...
    if err != nil {
        if cursor != nil && isInvalidCursorQuery(err) {
            return offeringdomain.ListResult{}, offeringdomain.ErrInvalidCursor
        }
        return offeringdomain.ListResult{}, fmt.Errorf("list offerings: %w", err)
    }
    defer rows.Close()

    result := offeringdomain.ListResult{Offerings: make([]offeringdomain.Offering, 0, limit), Limit: limit}
    for rows.Next() {
        value, scanErr := scanOffering(rows)
        if scanErr != nil {
            return offeringdomain.ListResult{}, fmt.Errorf("scan offering list row: %w", scanErr)
        }
        result.Offerings = append(result.Offerings, value)
    }
    if err := rows.Err(); err != nil {
        return offeringdomain.ListResult{}, fmt.Errorf("iterate offering list: %w", err)
    }
    if len(result.Offerings) > limit {
        last := result.Offerings[limit-1]
        next, encodeErr := offeringdomain.EncodeCursor(offeringdomain.Cursor{Version: 1, Code: last.Code, ID: last.ID})
        if encodeErr != nil {
            return offeringdomain.ListResult{}, encodeErr
        }
...
func (repository *Repository) ListWithAvailability(ctx context.Context, eventID string, input offeringdomain.ListInput) (offeringdomain.CatalogueResult, error) {
    limit, cursor, err := offeringdomain.NormalizeListInput(input)
    if err != nil {
    ...
    if err != nil {
        if cursor != nil && isInvalidCursorQuery(err) {
            return offeringdomain.CatalogueResult{}, offeringdomain.ErrInvalidCursor
        }
        return offeringdomain.CatalogueResult{}, fmt.Errorf("list offerings with availability: %w", err)
    }
    defer rows.Close()

    result := offeringdomain.CatalogueResult{Offerings: make([]offeringdomain.CatalogueOffering, 0, limit), Limit: limit}
    for rows.Next() {
        value, scanErr := scanCatalogueOffering(rows)
        if scanErr != nil {
            return offeringdomain.CatalogueResult{}, fmt.Errorf("scan offering availability row: %w", scanErr)
        }
        result.Offerings = append(result.Offerings, value)
    }
    if err := rows.Err(); err != nil {
        return offeringdomain.CatalogueResult{}, fmt.Errorf("iterate offering availability rows: %w", err)
    }
    if len(result.Offerings) > limit {
        last := result.Offerings[limit-1].Offering
        next, encodeErr := offeringdomain.EncodeCursor(offeringdomain.Cursor{Version: 1, Code: last.Code, ID: last.ID})
        if encodeErr != nil {
            return offeringdomain.CatalogueResult{}, encodeErr
        }
        result.Offerings = result.Offerings[:limit]
        result.NextCursor = next
    }
...
func (repository *Repository) GetWithAvailability(ctx context.Context, id string) (offeringdomain.CatalogueOffering, error) {
    value, err := scanCatalogueOffering(repository.db.QueryRowContext(ctx, operationsDetailQuery, id))
    if errors.Is(err, sql.ErrNoRows) {
        return offeringdomain.CatalogueOffering{}, offeringdomain.ErrNotFound
    }
    if err != nil {
        return offeringdomain.CatalogueOffering{}, fmt.Errorf("get offering with availability: %w", err)
    }
    return value, nil
}

func (repository *Repository) Save(ctx context.Context, value offeringdomain.Offering, expectedVersion int64) (offeringdomain.Offering, error) {
    ...
    if errors.Is(err, sql.ErrNoRows) {
        return offeringdomain.Offering{}, repository.classifyMissingOrStale(ctx, value.ID)
    }
    if err != nil {
        return offeringdomain.Offering{}, fmt.Errorf("save offering: %w", translateError(err))
    }
    return saved, nil
}

func (repository *Repository) ListPublic(ctx context.Context, eventID string, input offeringdomain.ListInput) (offeringdomain.CatalogueResult, error) {
    limit, cursor, err := offeringdomain.NormalizeListInput(input)
    if err != nil {
        return offeringdomain.CatalogueResult{}, err
    }
    ...
    if err != nil {
        ...
        return offeringdomain.CatalogueResult{}, fmt.Errorf("list public offerings: %w", err)
    }
    defer rows.Close()
    ...
    for rows.Next() {
        value, scanErr := scanCatalogueOffering(rows)
        if scanErr != nil {
            return offeringdomain.CatalogueResult{}, fmt.Errorf("scan public offering list row: %w", scanErr)
        }
        result.Offerings = append(result.Offerings, value)
    }
    if err := rows.Err(); err != nil {
        return offeringdomain.CatalogueResult{}, fmt.Errorf("iterate public offering list: %w", err)
    }
    if len(result.Offerings) > limit {
        last := result.Offerings[limit-1].Offering
        next, encodeErr := offeringdomain.EncodeCursor(offeringdomain.Cursor{Version: 1, Code: last.Code, ID: last.ID})
...
func (repository *Repository) GetPublic(ctx context.Context, id string) (offeringdomain.CatalogueOffering, error) {
    value, err := scanCatalogueOffering(repository.db.QueryRowContext(ctx, catalogueDetailQuery, id))
    if errors.Is(err, sql.ErrNoRows) {
        return offeringdomain.CatalogueOffering{}, offeringdomain.ErrNotFound
    }
    if err != nil {
        return offeringdomain.CatalogueOffering{}, fmt.Errorf("get public offering: %w", err)
    }
    return value, nil
}

func (repository *Repository) get(ctx context.Context, query, id string) (offeringdomain.Offering, error) {
    value, err := scanOffering(repository.db.QueryRowContext(ctx, query, id))
    if errors.Is(err, sql.ErrNoRows) {
        return offeringdomain.Offering{}, offeringdomain.ErrNotFound
    }
    if err != nil {
        return offeringdomain.Offering{}, fmt.Errorf("get offering: %w", err)
    }
    return value, nil
}
...
func (repository *Repository) requireActiveEvent(ctx context.Context, eventID string) error {
    ...
    if !active {
        return offeringdomain.ErrNotFound
    }
    return nil
}
...
func (repository *Repository) classifyMissingOrStale(ctx context.Context, id string) error {
    ...
    return offeringdomain.ErrStaleVersion
}

func scanOffering(scanner offeringScanner) (offeringdomain.Offering, error) {
    var row offeringRow
    if err := scanner.Scan(row.destinations()...); err != nil {
        return offeringdomain.Offering{}, err
    }
    return row.offering(), nil
}

func scanCatalogueOffering(scanner offeringScanner) (offeringdomain.CatalogueOffering, error) {
    var row offeringRow
    ...
    value := row.offering()
    availability, err := offeringdomain.AvailableParticipantUnits(offeringdomain.AvailabilityInput{
        EventQuota:    nullableInt64(eventQuota),
        OfferingQuota: value.ParticipantQuota,
        EventUsage:    offeringdomain.ReservationUsage{Reserved: eventReserved, Consumed: eventConsumed},
        OfferingUsage: offeringdomain.ReservationUsage{Reserved: offeringReserved, Consumed: offeringConsumed},
    })
    if err != nil {
        return offeringdomain.CatalogueOffering{}, err
    }
    return offeringdomain.CatalogueOffering{Offering: value, AvailableParticipantUnits: availability}, nil
}
...
func (row offeringRow) offering() offeringdomain.Offering {
    value := offeringdomain.Offering{
        ID:                  row.ID,
        EventID:             row.EventID,
        Code:                row.Code,
        Name:                row.Name,
        ...
        Status:              offeringdomain.Status(row.Status),
        Version:             row.Version,
    }
...
func translateError(err error) error {
    ...
    case "23505":
        if postgresError.ConstraintName == "offerings_event_id_code_key" {
            return offeringdomain.ErrDuplicateCode
        }
    case "23503":
        if postgresError.ConstraintName == "offerings_event_id_fkey" {
            return offeringdomain.ErrInvalidParentState
        }
    case "23514":
        return offeringdomain.ErrInvalidInput
    }
    return err
}
