...
func TestRepositoryCreateAndGet(t *testing.T) {
    ...
    mock.ExpectQuery("INSERT INTO parties").WithArgs(identitydomain.PartyTypePerson, "Purchaser", email, phone).WillReturnRows(row)

    repository := NewRepository(database)
    created, err := repository.Create(context.Background(), identitydomain.Party{Type: identitydomain.PartyTypePerson, DisplayName: "Purchaser", Email: &email, Phone: &phone})
    if err != nil {
        t.Fatal(err)
    }
