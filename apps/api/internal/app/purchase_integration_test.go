...
func TestOperationsPurchaseReadsPostgreSQL(t *testing.T) {
    ...
    parties := identitypersistence.NewRepository(database)
    purchaser, err := identitydomain.NewParty(identitydomain.CreateInput{Type: identitydomain.PartyTypePerson, DisplayName: "Purchaser"})
    if err != nil {
        t.Fatal(err)
    }
    ...
    purchase, err := purchasingdomain.NewPurchase(purchasingdomain.CreateInput{
        ...
        Participants:    []purchasingdomain.PurchaseParticipantInput{{PartyID: &purchaserID, DisplayName: "Stored participant"}},
    ...
    if err := platformdb.Within(ctx, database, func(transaction *sql.Tx) error {
        created, createErr := purchasingpersistence.NewRepository(transaction).Create(ctx, purchase)
    ...
    cipher, err := idempotency.NewCipher([]idempotency.Key{{ID: "purchase-test", Value: make([]byte, 32)}})
    if err != nil {
        t.Fatal(err)
    }
    server, err := NewServer(":0", logger, config.PublicConfig{RateLimitPerMinute: 60, RateLimitBurst: 20}, Dependencies{
        ...
        Purchasing: purchasingmodule.NewModule(database, identitymodule.NewModule().Service(), cipher, logger),
    })
    if err != nil {
        t.Fatal(err)
    }
