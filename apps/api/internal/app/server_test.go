...
func TestHealthAndReadiness(t *testing.T) {
    ...
    for _, test := range tests {
        t.Run(test.name, func(t *testing.T) {
            server := NewServer(":0", test.ping, logger, nil)
...
func TestHealthRejectsUnsupportedMethod(t *testing.T) {
    logger := slog.New(slog.NewTextHandler(io.Discard, nil))
    server := NewServer(":0", pingFunc(func(context.Context) error { return nil }), logger, nil)
...
func TestRouterReturnsNotFoundForUnknownAndUnimplementedPublicRoutes(t *testing.T) {
    logger := slog.New(slog.NewTextHandler(io.Discard, nil))
    server := NewServer(":0", pingFunc(func(context.Context) error { return nil }), logger, nil)

    for _, path := range []string{"/missing", "/api/public/v1/events/active"} {
