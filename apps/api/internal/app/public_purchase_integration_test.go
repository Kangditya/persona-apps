...
func TestPublicCommonPurchaseCheckoutPostgreSQL(t *testing.T) {
    ...
    purchaseModule := purchasingmodule.NewModule(database, identity.Service(), cipher, logger)
    server, err := NewServer(":0", logger, config.PublicConfig{
        RateLimitPerMinute: 60, RateLimitBurst: 20, StorefrontAllowedOrigins: map[string]struct{}{origin: {}},
    }, Dependencies{Database: database, Purchasing: purchaseModule})
    if err != nil {
        t.Fatal(err)
    }
