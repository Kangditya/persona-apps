...
describe("Operations service-worker policy", () => {
  it("handles only immutable shell assets and data-free navigation fallback", () => {
    ...
    const dispatchFetch = (method: string, pathname: string, mode = "cors") => {
      ...
      listeners.get("fetch")?.({
        request: {
          method,
          mode,
          url: `https://operations.example.test${pathname}`,
        },
        respondWith,
      });
      return respondWith;
    ...
    expect(
      dispatchFetch("GET", "/api/operations/v1/auth/session"),
    ...
    expect(dispatchFetch("PATCH", "/events/example")).not.toHaveBeenCalled();
    expect(
      dispatchFetch("GET", "/_next/static/chunk.js"),
    ).toHaveBeenCalledOnce();
    expect(dispatchFetch("GET", "/events", "navigate")).toHaveBeenCalledOnce();
