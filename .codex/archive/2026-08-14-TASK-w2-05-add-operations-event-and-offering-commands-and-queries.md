...
execution. The accepted command replay retention is 24 hours.
...
- Planned behavior: normalized Event and Offering patches that do not change
  stored values must not bump versions or emit audit/outbox effects.
- Unexpected requirement: the existing domain services mark every supplied
  field as changed before normalization, so an HTTP-only check would leave
  sibling callers vulnerable to false mutations.
- Necessity and impact: update the existing Event and Offering services and
  their focused tests at the shared behavior boundary. This remains inside the
  approved W2-05 concurrency/audit objective and adds no new abstraction.
- Planned behavior: Operations Offering responses include advisory availability
  for all authorized statuses.
- Unexpected requirement: the existing repository only computes availability
  inside public active/published queries.
- Necessity and impact: extend the existing Offering repository with bounded
  Operations availability reads and focused PostgreSQL coverage; do not issue
  one query per list row or weaken public visibility filters.
- Planned verification: `make validate` must pass before archival.
- Unexpected requirement: the committed formatter baseline contained two
  pre-existing platform transaction files that failed the repository formatter.
- Necessity and impact: normalize only those two files with the existing Go
  formatter. `git diff -w --exit-code` confirms the change is whitespace-only;
  no transaction behavior or test expectation changed.
...
- Confirmed default: nullable patch fields distinguish omission from explicit
  clearing.
- Approved execution decision: Event/Offering create and lifecycle commands use
  the proposed 24-hour replay window.
