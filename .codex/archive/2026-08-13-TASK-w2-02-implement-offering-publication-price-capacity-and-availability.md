...
Executed and verified for the framework-neutral Offering domain boundary on
2026-08-13.
...
- Added the framework-neutral `internal/offering` model with exact minor-unit
  pricing, uppercase currency validation, bounded capacity/quota, immutable
  structural fields, lifecycle guards, first-publication timestamps, and
  nullable patch-clearing inputs.
- Added advisory availability calculation: only `RESERVED` and `CONSUMED`
  units reduce a finite Event or Offering quota; the finite minimum is clamped
  at zero and fully unbounded availability is `null`.
- Added minimized audit-state/action/outbox/retry metadata for later atomic
  command composition and updated `.codex/CURRENT_STATE.md`.

### Verified

- Focused Event/Offering format checks, `go vet`, and domain tests pass.
- Full API `go test ./...` and `go build ./...` pass with both modules.

### Assumed

- A non-terminal Event can own a draft Offering; only `PUBLISHED`, `ACTIVE`,
  or `SUSPENDED` permits publication, while cleanup transitions remain allowed
  after Event closure.

### Deferred

- PostgreSQL availability aggregation, transaction wiring, quota reservation
  mutation, Purchase price snapshots, audit/outbox persistence, authorization,
  idempotency replay, and all HTTP routes remain outside this task.

### Remaining Risks and Next Task

- Availability is intentionally advisory until a checkout command locks and
  revalidates the authoritative totals.
- Next: W2-03 persists both domain models, version checks, visibility queries,
  and availability aggregation in PostgreSQL.
