...
execution. No production auth bypass or browser-stored token was added.
...
- The authenticated browser smoke used the real Go API and PostgreSQL with a
  disposable operator plus a temporary local OIDC verifier. The verifier and
  credentials remain outside the repository; a production provider was not
  exercised.
- Browser checks covered create, edit, lifecycle, stale-version recovery,
  rotated-CSRF recovery, direct route loading, logout, semantic form controls,
  and a 360-pixel layout. Datetime conversion is covered by focused round-trip
  tests; no browser-specific nonexistent-local-time case was forced manually.
- No new frontend dependency, auth store, generated client, form framework, or
  optimistic write path was added.
