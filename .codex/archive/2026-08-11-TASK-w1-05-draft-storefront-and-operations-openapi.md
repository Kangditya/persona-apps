...
Checkout returns a raw Purchase bearer token while idempotent replay returns
the committed response. ADR-045 therefore requires an encrypted response
envelope at rest. This task records the decision in Architecture, Decisions,
Authentication, and ERD documentation; W1-06 implements it. No runtime,
dependency, migration, frontend, client, or infrastructure scope is added.
...
| `docs/DECISIONS.md` | Modify | Record encrypted-at-rest replay behavior for credential-bearing responses. |
| `docs/security/AUTHENTICATION.md` | Modify | Bind Purchase-token replay to the encrypted response-envelope decision. |
| `docs/database/ERD.md` | Modify | Clarify encrypted response-body persistence. |
...
- Separate Phase 1 Storefront and Operations OpenAPI 3.1 contracts, including
  lifecycle commands, permissions, request IDs, bounded cursors, CSRF, bearer
  access, idempotency, evidence limits, and common error envelopes.
- ADR-045 encrypted-at-rest replay policy for credential-bearing successful
  responses, with Architecture, Authentication, ERD, and current-state updates.

### Verified

- Prettier formatting and the pinned Redocly parser completed with no OpenAPI
  errors. Its seven non-blocking recommendations are missing repository legal
  metadata, two intentional 302 OIDC redirects, and unused Operations-only
  response components.
- make validate passed.
- Storefront exposure search found no token hashes, session hashes, CSRF
  hashes, permission snapshots, evidence references, or provider references.

### Assumed

- The configured key ring is retained for at least the longest replay window;
  deployment-specific key values and retention remain W1-06/W1-07 configuration.

### Deferred

- Runtime handlers, generated clients, Storefront queries, private evidence
  viewing, Saving, Giveaway, livestock, allocation, refunds, webhooks, and
  rate-limit implementation.

### Next Task

Implement W1-06 API platform foundations, including the ADR-045 encrypted
replay envelope and the authentication boundary.
