...
Completed and verified on 2026-09-04 after explicit execution approval and a
fresh Sol `ship` verdict. The live tracker remains `Backlog`; no tracker,
commit, or push change was requested.

## Execution Reconciliation (2026-09-04)

- The complete approved plan is preserved below as the execution baseline.
- ADR-057 selects one standard-library private filesystem `EvidenceStore`, not
  an object provider, and defines its single-instance deployment ceiling,
  consistency cleanup, private retrieval, configuration, retention owner, and
  future provider migration path.
- This activation/documentation pass precedes code BUILD. It does not change
  code, tests, contracts, configuration source, tracker, archive, commit, or
  push state.

## Execution Verification (2026-09-04)

- ADR-057 approves the standard-library, single-instance private filesystem
  mechanism. Configuration names only: `EVIDENCE_STORAGE_ROOT`,
  `EVIDENCE_STORAGE_MODE=single-instance`, and `API_REPLICA_COUNT=1`.
- `os.Root` confines storage; directories use `0700`, files and leases use
  `0600`, opaque keys have 256 bits of entropy, and sync/link finalization
  supplies the accepted durability boundary.
- Authorization precedes spool/replay. Exact 10 MiB plus one sentinel byte
  streaming, definite rollback and panic cleanup, commit-uncertainty/orphan
  handling, and hourly cleanup after a 24-hour grace period are covered.
- Private retrieval requires `payment.verify` and supplies safe headers and
  CORS behavior.
- Isolated PostgreSQL 18 migrations reached version 7 with `dirty=false`.
  Focused, race, and full Go tests; formatter, vet, and build; Redocly with
  zero errors and six pre-existing warnings; `make validate`; Compose config;
  and diff/leak checks all passed.
- Fresh Sol final verdict: `ship`, with no findings.
- Deferred outside W4-02: honest single-replica/private-volume deployment,
  backup/restore, referenced-evidence retention, legal hold/deletion authority,
  and a future multi-replica provider.
